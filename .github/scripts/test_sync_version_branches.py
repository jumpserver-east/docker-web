...
class SyncVersionBranchesTests(unittest.TestCase):
    ...
    def test_wrong_origin_is_rejected_before_any_remote_access(self):
        self.env['EXPECTED_ORIGIN_REPOSITORY'] = 'jumpserver-east/docker-web'
        for url in ('https://github.com/jumpserver/docker-web.git',
                    'git@github.com:jumpserver-east/koko.git'):
            with self.subTest(url=url):
                self.git(self.work, 'config', 'remote.origin.pushurl', url)
                result, summary = self.sync('false')
                self.assertNotEqual(result.returncode, 0)
                self.assertIn('Refusing to sync: origin must push only to jumpserver-east/docker-web', result.stderr)
                self.assertEqual(summary, '')

    def test_multiple_push_destinations_are_rejected(self):
        self.env['EXPECTED_ORIGIN_REPOSITORY'] = 'jumpserver-east/docker-web'
        self.git(self.work, 'config', '--add', 'remote.origin.pushurl',
                 'https://github.com/jumpserver-east/docker-web.git')
        self.git(self.work, 'config', '--add', 'remote.origin.pushurl',
                 'https://github.com/jumpserver/docker-web.git')
        result, summary = self.sync('false')
        self.assertNotEqual(result.returncode, 0)
        self.assertIn('Refusing to sync', result.stderr)
        self.assertEqual(summary, '')
