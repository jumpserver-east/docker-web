...
UPSTREAM_URL="${UPSTREAM_URL:-https://github.com/jumpserver/docker-web.git}"
...
# The Actions workflow supplies the fork name. Check the effective push URL,
# including pushurl/url rewrites; multiple push destinations are rejected too.
if [[ -n "${EXPECTED_ORIGIN_REPOSITORY:-}" ]]; then
  origin_push_url="$(git remote get-url --push --all origin)"
  case "${origin_push_url%.git}" in
    "https://github.com/${EXPECTED_ORIGIN_REPOSITORY}"|\
    "git@github.com:${EXPECTED_ORIGIN_REPOSITORY}"|\
    "ssh://git@github.com/${EXPECTED_ORIGIN_REPOSITORY}") ;;
    *)
      echo "Refusing to sync: origin must push only to ${EXPECTED_ORIGIN_REPOSITORY}." >&2
      exit 1
      ;;
  esac
fi
